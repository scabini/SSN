...
{	
	if (nrhs != 3) {
		...
		mexErrMsgTxt("outputs 1 numeric matrix 4x(w*h*z): measures (k and str) for FULL, WITHIN and BETWEEN network connections");
	}
    int w, h, z, np;	  
    double *im;
    int radius, L;
	...
	plhs[0] = mxCreateDoubleMatrix(4, np, mxREAL);//single network N for grayscale   
